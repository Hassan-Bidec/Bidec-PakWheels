...
const Page = () => {
  ...
  useEffect(() => {
    if (slug) {
      setIsLoading(true);
      setError(null);
      listingsApi.getListingBySlug(slug)
        .then((res) => {
          setListing(res?.listing);
          setIsLoading(false);
        })
        .catch((err) => {
          const errorMessage = extractErrorMessage(err, "Failed to fetch listing.");
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          <h2 className="text-xl font-semibold text-red-800 mb-2">
            {t("Error Loading Listing")}
          </h2>
          <p className="text-red-600">{error}</p>
          <button
            className="mt-4 px-4 py-2 rounded bg-green-600 text-white text-sm hover:bg-green-700 transition-colors cursor-pointer"
  ...
  async function handleWithdraw(option) {
    try {
      ...
      const errorMessage = extractErrorMessage(error, "Failed to withdraw listing.");
      toast.error(errorMessage);
    }
  }
